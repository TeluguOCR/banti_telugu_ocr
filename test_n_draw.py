...
import mallidraw as draw
import theano
import time
import codecs

#sys.stdout = codecs.getwriter('utf8')(sys.stdout)
def enc(a): return a.encode('utf-8')
def dec(a): return a #a.decode('utf-8')
def UnicodeToStrDecorator(psseFunc):
    def callPsseFunc(*args, **kwargs):
        ...
        for a in args:
            if type(a) is unicode:  newArgs.append(enc(a))
            else:                   newArgs.append(dec(a))
        for k, v in kwargs.items():
            if type(v) is unicode:  newKwargs[k] = enc(v)
            else:                   newKwargs[k] = dec(v)
        # Call origional psse function with new, sanitized arguments
        return psseFunc(*newArgs, **newKwargs)
    return callPsseFunc

print = UnicodeToStrDecorator(print)    

def printf(s, *args):
    print(enc(s).format(*args))

def format(s, *args):
    return UnicodeToStrDecorator(enc(s).format)(*args)

def join(s, *args):
    return UnicodeToStrDecorator(enc(s).join)(*args)
...
def mkdir(direc):
    ...
    try: 
        ...
        if os.path.isdir(direc) : print('Exits')
        else: raise exc
mkdir(output_dir)

css_loc = os.path.join(output_dir, 'collapse.css')
printf('Home: {} \nInput:{} \nOutput:{} \nCSS:{}', home, dirs_dir, output_dir, css_loc)
...
class Index():
    ...
    def add_good(self, i):        self.good.append(i)
    def add_bad (self, i):        self.bad.append(i)
    def add_subdir(self, i):      self.dirs.append(i)
    ...
    def close(self):
        ...
        try: 
            summary = format('Total: {} Correct:{}({:.2f}%) Wrong:{}({:.2f}%)',
                total, enc, 100*float(enc)/total, wrng, 100*float(wrng)/total)
        except ZeroDivisionError: summary = 'Characters'
...
for dirpath, dirnames, filenames in os.walk(dirs_dir):
    ...
    for dirname in sorted(dirnames):
        uni = unicodes[char_indices[dirname]] if dirname in char_indices else dirname
        print(uni, dirname)
